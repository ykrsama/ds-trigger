...
// GroupNorm3D implementation for Input Conv Block (after first conv: 32 channels)
void InputGroupNorm3D_1(
    ...
    const int CHANNELS_PER_GROUP = F_MAP_h / NUM_GROUPS;
    const float N = (float)(INPUT_DEPTH * INPUT_HEIGHT * INPUT_WIDTH * CHANNELS_PER_GROUP);
    ...
    for (int batch = 0; batch < BATCH_SIZE; batch++) {
            ...
            for (int depth = 0; depth < INPUT_DEPTH; depth++) {
                for (int height = 0; height < INPUT_HEIGHT; height++) {
                    for (int width = 0; width < INPUT_WIDTH; width++) {
                        ...
                        float value = input[batch][ch][depth][height][width];
        ...
        // Calculate mean and inv_std for each group
            ...
            for (int height = 0; height < INPUT_HEIGHT; height++) {
                for (int width = 0; width < INPUT_WIDTH; width++) {
                    ...
                    for (int ch = 0; ch < F_MAP_h; ch++) {
                        ...
                        output[batch][ch][depth][height][width] = output_value;
                    }
                }
            }
        }
...
// GroupNorm3D implementation for Input Conv Block (after second conv: 64 channels)
void InputGroupNorm3D_2(
    ...
    const int CHANNELS_PER_GROUP = F_MAP_0 / NUM_GROUPS;
    const float N = (float)(INPUT_DEPTH * INPUT_HEIGHT * INPUT_WIDTH * CHANNELS_PER_GROUP);
...
// GroupNorm3D implementation for Encoder Conv Block (after first conv: 64 channels, pooled size)
void EncoderGroupNorm3D_1(
    ...
    const int CHANNELS_PER_GROUP = F_MAP_0 / NUM_GROUPS;
    const float N = (float)(POOL_OUTPUT_DEPTH * POOL_OUTPUT_HEIGHT * POOL_OUTPUT_WIDTH * CHANNELS_PER_GROUP);
...
// GroupNorm3D implementation for Encoder Conv Block (after second conv: 128 channels, pooled size)
void EncoderGroupNorm3D_2(
    ...
    const int CHANNELS_PER_GROUP = F_MAP_1 / NUM_GROUPS;
    const float N = (float)(POOL_OUTPUT_DEPTH * POOL_OUTPUT_HEIGHT * POOL_OUTPUT_WIDTH * CHANNELS_PER_GROUP);

    float gn_buffer[BATCH_SIZE][F_MAP_1][POOL_OUTPUT_DEPTH][POOL_OUTPUT_HEIGHT][POOL_OUTPUT_WIDTH];
    #pragma HLS bind_storage variable=gn_buffer type=ram_2p impl=uram
...
// GroupNorm3D implementation for Decoder Conv Block (after first conv: 64 channels, full size)
...
// GroupNorm3D implementation for Decoder Conv Block (after second conv: 64 channels, full size)
...
// GroupNorm3D implementation for Output Conv Block (after first conv: 64 channels, full size)
...
// GroupNorm3D implementation for Output Conv Block (after second conv: 64 channels, full size)
