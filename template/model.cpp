...
void GroupNorm3D(data_t input_data[BATCH_SIZE][T_IN_CHANNELS][T_INPUT_DEPTH][T_INPUT_HEIGHT][T_INPUT_WIDTH],
    ...
    const data_t N = (data_t) (T_INPUT_DEPTH * T_INPUT_HEIGHT * T_INPUT_WIDTH * CHANNELS_PER_GROUP);
    ...
    data_t group_sq_sum[NUM_GROUPS];
    ...
    for (int g = 0; g < NUM_GROUPS; g++) {
        #pragma HLS unroll
        group_sum[g] = (data_t)0.0;
        group_sq_sum[g] = (data_t)0.0;
    }
            ...
            for (int height = 0; height < T_INPUT_HEIGHT; height++) {
                ...
                for (int width = 0; width < T_INPUT_WIDTH; width++) {
                    ...
                    for (int ch = 0; ch < T_IN_CHANNELS; ch++) {
                        ...
                        data_t value = gn_buffer[batch][ch][depth][height][width];

                        // Parameters
                        data_t gamma_param = gamma[ch];
                        data_t beta_param = beta[ch];
                        data_t group_mean = mean[group_idx];
                        data_t group_inv_std = inv_std[group_idx];

                        // Normalize
                        data_t normalized_value = (value - group_mean) * group_inv_std;
                        data_t output_value = normalized_value * gamma_param + beta_param;
                                        ...
                                        for (int kd = 0; kd < CONV_KERNEL; kd++) {
                                            for (int kh = 0; kh < CONV_KERNEL; kh++) {
                                                for (int kw = 0; kw < CONV_KERNEL; kw++) {
                                                    data_t window_val = window_buffer[batch][in_ch][kd][kh][kw];
                                                    data_t kernel_val = kernel[out_ch][in_ch][kd][kh][kw];
...
void Upsample3D(data_t input[BATCH_SIZE][T_IN_CHANNELS][POOL_OUTPUT_DEPTH][POOL_OUTPUT_HEIGHT][POOL_OUTPUT_WIDTH],
    ...
    data_t line_buffer[T_IN_CHANNELS][POOL_OUTPUT_WIDTH];
...
void Sigmoid3D(data_t input[BATCH_SIZE][OUT_CHANNELS][INPUT_DEPTH][INPUT_HEIGHT][INPUT_WIDTH],
    ...
    static const data_t sigmoid_lut[97] = {
            ...
            for (int depth = 0; depth < INPUT_DEPTH; depth++) {
                for (int height = 0; height < INPUT_HEIGHT; height++) {
                    for (int width = 0; width < INPUT_WIDTH; width++) {
                        ...
                        data_t x = input[batch][ch][depth][height][width];
                        ...
                        if (x <= data_t(-6.0)) {
                            ...
                            data_t y0 = sigmoid_lut[index];
                            data_t y1 = sigmoid_lut[index + 1];
...
template void GroupNorm3D<IN_CHANNELS>(data_t[BATCH_SIZE][IN_CHANNELS][INPUT_DEPTH][INPUT_HEIGHT][INPUT_WIDTH], data_t[IN_CHANNELS], data_t[IN_CHANNELS], data_t[BATCH_SIZE][IN_CHANNELS][INPUT_DEPTH][INPUT_HEIGHT][INPUT_WIDTH]);
template void GroupNorm3D<F_MAP_h>(data_t[BATCH_SIZE][F_MAP_h][INPUT_DEPTH][INPUT_HEIGHT][INPUT_WIDTH], data_t[F_MAP_h], data_t[F_MAP_h], data_t[BATCH_SIZE][F_MAP_h][INPUT_DEPTH][INPUT_HEIGHT][INPUT_WIDTH]);
template void GroupNorm3D<F_MAP_0>(data_t[BATCH_SIZE][F_MAP_0][INPUT_DEPTH][INPUT_HEIGHT][INPUT_WIDTH], data_t[F_MAP_0], data_t[F_MAP_0], data_t[BATCH_SIZE][F_MAP_0][INPUT_DEPTH][INPUT_HEIGHT][INPUT_WIDTH]);
template void GroupNorm3D<F_MAP_0, POOL_OUTPUT_DEPTH, POOL_OUTPUT_HEIGHT, POOL_OUTPUT_WIDTH>(data_t[BATCH_SIZE][F_MAP_0][POOL_OUTPUT_DEPTH][POOL_OUTPUT_HEIGHT][POOL_OUTPUT_WIDTH], data_t[F_MAP_0], data_t[F_MAP_0], data_t[BATCH_SIZE][F_MAP_0][POOL_OUTPUT_DEPTH][POOL_OUTPUT_HEIGHT][POOL_OUTPUT_WIDTH]);
template void GroupNorm3D<F_MAP_1, POOL_OUTPUT_DEPTH, POOL_OUTPUT_HEIGHT, POOL_OUTPUT_WIDTH>(data_t[BATCH_SIZE][F_MAP_1][POOL_OUTPUT_DEPTH][POOL_OUTPUT_HEIGHT][POOL_OUTPUT_WIDTH], data_t[F_MAP_1], data_t[F_MAP_1], data_t[BATCH_SIZE][F_MAP_1][POOL_OUTPUT_DEPTH][POOL_OUTPUT_HEIGHT][POOL_OUTPUT_WIDTH]);
template void GroupNorm3D<CONCAT_CHANNELS>(data_t[BATCH_SIZE][CONCAT_CHANNELS][INPUT_DEPTH][INPUT_HEIGHT][INPUT_WIDTH], data_t[CONCAT_CHANNELS], data_t[CONCAT_CHANNELS], data_t[BATCH_SIZE][CONCAT_CHANNELS][INPUT_DEPTH][INPUT_HEIGHT][INPUT_WIDTH]);

template void Conv3D<IN_CHANNELS, F_MAP_h>(data_t[F_MAP_h][IN_CHANNELS][CONV_KERNEL][CONV_KERNEL][CONV_KERNEL], data_t[BATCH_SIZE][IN_CHANNELS][INPUT_DEPTH][INPUT_HEIGHT][INPUT_WIDTH], data_t[BATCH_SIZE][F_MAP_h][INPUT_DEPTH][INPUT_HEIGHT][INPUT_WIDTH]);
template void Conv3D<F_MAP_h, F_MAP_0>(data_t[F_MAP_0][F_MAP_h][CONV_KERNEL][CONV_KERNEL][CONV_KERNEL], data_t[BATCH_SIZE][F_MAP_h][INPUT_DEPTH][INPUT_HEIGHT][INPUT_WIDTH], data_t[BATCH_SIZE][F_MAP_0][INPUT_DEPTH][INPUT_HEIGHT][INPUT_WIDTH]);
template void Conv3D<F_MAP_0, F_MAP_0, POOL_OUTPUT_DEPTH, POOL_OUTPUT_HEIGHT, POOL_OUTPUT_WIDTH>(data_t[F_MAP_0][F_MAP_0][CONV_KERNEL][CONV_KERNEL][CONV_KERNEL], data_t[BATCH_SIZE][F_MAP_0][POOL_OUTPUT_DEPTH][POOL_OUTPUT_HEIGHT][POOL_OUTPUT_WIDTH], data_t[BATCH_SIZE][F_MAP_0][POOL_OUTPUT_DEPTH][POOL_OUTPUT_HEIGHT][POOL_OUTPUT_WIDTH]);
...
// Input Path
template void DoubleConv3D2Head<IN_CHANNELS, F_MAP_h, F_MAP_0>(data_t[BATCH_SIZE][IN_CHANNELS][INPUT_DEPTH][INPUT_HEIGHT][INPUT_WIDTH], data_t[F_MAP_h][IN_CHANNELS][CONV_KERNEL][CONV_KERNEL][CONV_KERNEL], data_t[IN_CHANNELS], data_t[IN_CHANNELS], data_t[F_MAP_0][F_MAP_h][CONV_KERNEL][CONV_KERNEL][CONV_KERNEL], data_t[F_MAP_h], data_t[F_MAP_h], data_t[BATCH_SIZE][F_MAP_0][INPUT_DEPTH][INPUT_HEIGHT][INPUT_WIDTH], data_t[BATCH_SIZE][F_MAP_0][INPUT_DEPTH][INPUT_HEIGHT][INPUT_WIDTH]);
// Encoder Path
template void MaxPool3D<F_MAP_0>(data_t[BATCH_SIZE][F_MAP_0][INPUT_DEPTH][INPUT_HEIGHT][INPUT_WIDTH], data_t[BATCH_SIZE][F_MAP_0][POOL_OUTPUT_DEPTH][POOL_OUTPUT_HEIGHT][POOL_OUTPUT_WIDTH]);
template void DoubleConv3D<F_MAP_0, F_MAP_0, F_MAP_1, POOL_OUTPUT_DEPTH, POOL_OUTPUT_HEIGHT, POOL_OUTPUT_WIDTH>(data_t[BATCH_SIZE][F_MAP_0][POOL_OUTPUT_DEPTH][POOL_OUTPUT_HEIGHT][POOL_OUTPUT_WIDTH], data_t[F_MAP_0][F_MAP_0][CONV_KERNEL][CONV_KERNEL][CONV_KERNEL], data_t[F_MAP_0], data_t[F_MAP_0], data_t[F_MAP_1][F_MAP_0][CONV_KERNEL][CONV_KERNEL][CONV_KERNEL], data_t[F_MAP_0], data_t[F_MAP_0], data_t[BATCH_SIZE][F_MAP_1][POOL_OUTPUT_DEPTH][POOL_OUTPUT_HEIGHT][POOL_OUTPUT_WIDTH]);
// Decoder Path
template void Upsample3D<F_MAP_1>(data_t[BATCH_SIZE][F_MAP_1][POOL_OUTPUT_DEPTH][POOL_OUTPUT_HEIGHT][POOL_OUTPUT_WIDTH], data_t[BATCH_SIZE][F_MAP_1][INPUT_DEPTH][INPUT_HEIGHT][INPUT_WIDTH]);
template void ConcatenateTensors<F_MAP_0, F_MAP_1, CONCAT_CHANNELS>(data_t[BATCH_SIZE][F_MAP_0][INPUT_DEPTH][INPUT_HEIGHT][INPUT_WIDTH], data_t[BATCH_SIZE][F_MAP_1][INPUT_DEPTH][INPUT_HEIGHT][INPUT_WIDTH], data_t[BATCH_SIZE][CONCAT_CHANNELS][INPUT_DEPTH][INPUT_HEIGHT][INPUT_WIDTH]);
template void DoubleConv3D<CONCAT_CHANNELS, F_MAP_0, F_MAP_0>(data_t[BATCH_SIZE][CONCAT_CHANNELS][INPUT_DEPTH][INPUT_HEIGHT][INPUT_WIDTH], data_t[F_MAP_0][CONCAT_CHANNELS][CONV_KERNEL][CONV_KERNEL][CONV_KERNEL], data_t[CONCAT_CHANNELS], data_t[CONCAT_CHANNELS], data_t[F_MAP_0][F_MAP_0][CONV_KERNEL][CONV_KERNEL][CONV_KERNEL], data_t[F_MAP_0], data_t[F_MAP_0], data_t[BATCH_SIZE][F_MAP_0][INPUT_DEPTH][INPUT_HEIGHT][INPUT_WIDTH]);
// Output Path
template void DoubleConv3D<F_MAP_0, F_MAP_0, F_MAP_0>(data_t[BATCH_SIZE][F_MAP_0][INPUT_DEPTH][INPUT_HEIGHT][INPUT_WIDTH], data_t[F_MAP_0][F_MAP_0][CONV_KERNEL][CONV_KERNEL][CONV_KERNEL], data_t[F_MAP_0], data_t[F_MAP_0], data_t[F_MAP_0][F_MAP_0][CONV_KERNEL][CONV_KERNEL][CONV_KERNEL], data_t[F_MAP_0], data_t[F_MAP_0], data_t[BATCH_SIZE][F_MAP_0][INPUT_DEPTH][INPUT_HEIGHT][INPUT_WIDTH]);
template void FinalConv1x1<F_MAP_0, OUT_CHANNELS>(data_t[OUT_CHANNELS][F_MAP_0][1][1][1], data_t[OUT_CHANNELS], data_t[BATCH_SIZE][F_MAP_0][INPUT_DEPTH][INPUT_HEIGHT][INPUT_WIDTH], data_t[BATCH_SIZE][OUT_CHANNELS][INPUT_DEPTH][INPUT_HEIGHT][INPUT_WIDTH]);
                    ...
                    if (width >= CONV_KERNEL - 1 &&
                        ...
                        for (int out_ch = 0; out_ch < T_OUT_CHANNELS; out_ch++) {
                            ...
                            for (int in_ch = 0; in_ch < T_IN_CHANNELS; in_ch++) {
                                for (int kh = 0; kh < CONV_KERNEL; kh++) {
                                    for (int kw = 0; kw < CONV_KERNEL; kw++) {
                                        data_t window_val = window_buffer[batch][in_ch][kh][kw];
                                        data_t kernel_val = kernel[out_ch][in_ch][kh][kw];
                                        accum[out_ch] += window_val * kernel_val;
                                    }
                                }
                            }
...
// Instantiate Conv2D
template void Conv2D<INPUT_DEPTH, F_MAP_h, INPUT_HEIGHT, INPUT_WIDTH>(data_t[F_MAP_h][INPUT_DEPTH][CONV_KERNEL][CONV_KERNEL],
                                                                      data_t[BATCH_SIZE][INPUT_DEPTH][INPUT_HEIGHT][INPUT_WIDTH],
                                                                      data_t[BATCH_SIZE][F_MAP_h][INPUT_HEIGHT][INPUT_WIDTH]);


// OutputPath
template void FinalConv1x1<IN_CHANNELS, OUT_CHANNELS>(data_t[OUT_CHANNELS][IN_CHANNELS][1][1][1],
                                                     data_t[OUT_CHANNELS],
                                                     data_t[BATCH_SIZE][IN_CHANNELS][INPUT_DEPTH][INPUT_HEIGHT][INPUT_WIDTH],
